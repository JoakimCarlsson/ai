...
Entries keep their Go constant name, their ID and any field the API does not
publish, so regenerating never renames an exported identifier. A model the API
stops serving is removed, which drops its constant: check the removal report and
call the break out in your commit message.

Adding a provider means adding one adapter in `cmd/modelsync`. Catalogs without
the banner are still maintained by hand.
